# ─────────────────────────────────────────────────────────────────────────────
# German Data Protection MCP — multi-stage Dockerfile
# ─────────────────────────────────────────────────────────────────────────────
# Build:  docker build -t german-data-protection-mcp .
# Run:    docker run --rm -p 3000:3000 german-data-protection-mcp
#
# The image bakes /app/data/bfdi.db at build time. Override BFDI_DB_PATH
...
ENV BFDI_DB_PATH=/app/data/bfdi.db
...
# Bake the pre-built database into the image so /app/data/bfdi.db resolves
# at runtime without a bind mount. This matches the working pattern in
# swedish-data-protection-mcp's GHCR image.
#
# The explicit `data/bfdi.db` reference is required — `.github/workflows/
...
# We then COPY it into the image at /app/data/bfdi.db (BFDI_DB_PATH). The
# explicit `data/<name>.db` reference is required for the workflow's grep
# `COPY\s+\K(data/\S+\.db)` to match.
COPY data/database.db data/bfdi.db
